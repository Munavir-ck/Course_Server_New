...
const login = async (req, res) => {
  try {
    ...
    if (user) {
      ...
      res
        .status(200)
        .json({ status: false, message: "User email is incorrect" });
    }
  } catch (error) {
    console.log(error);
    res.status(500).send("Internal Server Error");
    ...
    await teacherDb
      ...
      .then((data) => {
        if (data.length === 0) {
          ...
          res
            .status(200)
            .json({ status: false, message: "No available teachers" });
        } else {
